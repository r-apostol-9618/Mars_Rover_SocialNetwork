...
import com.example.nasa_project.data.MyAdapterSpiritRover
...
import com.example.nasa_project.data.api.ApiServiceSpiritObject
...
import kotlinx.android.synthetic.main.fragment_rover_spirit.*
...
class RoverSpiritFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_rover_spirit, container, false)
        ...
        picker.addOnPositiveButtonClickListener {
            ...
            val dataListSpiritRover: MutableList<Photo> = mutableListOf()
            lateinit var myAdapterSpiritRover: MyAdapterSpiritRover
            var listSpiritRover: ArrayList<Photo>? = null

            GlobalScope.launch(Dispatchers.Main) {

                val response = ApiServiceSpiritObject.RETROFIT_SERVICE.getPhotosSpirit(
                ...
                if(response.isSuccessful && response.body() != null){
                    try {
                        if (response.body()?.photos?.isEmpty()!!) {
                            ...
                            myAdapterSpiritRover =
                                MyAdapterSpiritRover(dataListSpiritRover)

                            recyclerViewSpiritRover.layoutManager = LinearLayoutManager(context)
                            recyclerViewSpiritRover.addItemDecoration(
                                DividerItemDecoration(
                                    context,
                                    OrientationHelper.VERTICAL
                                )
                            )
                            recyclerViewSpiritRover.adapter = myAdapterSpiritRover

                            listSpiritRover = (response.body()?.photos as ArrayList<Photo>?)!!
                            dataListSpiritRover.addAll(listSpiritRover!!)
                            myAdapterSpiritRover.notifyDataSetChanged()
                        }
                    } catch (e: Exception) {
                        e.printStackTrace()
                        val snackBar: Snackbar = Snackbar.make(
        ...
        view.txtViewRoverName.setOnClickListener {
            ...
            val fragment = RoverSpiritInfoFragment()
